...
class BetterStatsWebpackPlugin {
    ...
    apply(compiler) {
        compiler.hooks.afterEmit.tapAsync('BetterStatsWebpackPlugin', this.writeStats);
    }
        ...
        modules
            ...
            .forEach(m => {
                if (m.assets.length === 1) {
                    const src = path.resolve(compilation.options.context, m.name);
                    const key = path.relative(compilation.options.context, src);
